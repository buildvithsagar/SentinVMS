...
- `[ ]` D.1: Build color-coded Timeline Scrubber widget (Blue = Continuous, Orange = Motion, Green = Scheduled)
- `[ ]` D.2: Implement interactive seek pre-buffering (load signed HLS URLs on handle drop)
- `[ ]` D.3: Build Playback Speed controls (0.5x to 16x multiplier)
- `[ ]` D.4: Implement `AlarmBloc` capturing WebSocket events in foreground and FCM in background
- `[ ]` D.5: Implement Alarm List UI and Reconciliation on Resume API syncing
- `[ ]` D.6: Build Evidence Export Wizard UI (initiator, status poller, and platform share sheet)
- `[ ]` D.7: Write tests for timeline seeks, WebSocket alarms, and export poller flows
